...
function addUploadItem(type,path,memberDown){
	...
	try{
	  ...
	  EditType="FCkEditor"
	  var hrefLen=location.href.lastIndexOf("index.html")
...
function getTB(id){
	...
	tJS.src = "getValidateKey5eac.html?tbID=" + id + "&type=trackback&vcode=" + document.getElementById("vCode").value;
}
...
function setTBKey(code){
	if (!code || code == "codeError"){
		alert("验证码错误");
		document.getElementById("vCode").select();
		document.getElementById("tbCode").src = 'common/getCode1919.gif?s='+Math.random();
		return;
	}
...
function hideMask(){
	...
	if (vImg) vImg.src="common/getCode1919.gif?s="+Math.random();
}
